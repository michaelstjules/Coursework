...
void Inspector::addBloodEntity(const Type t)
{
  ...
  {
    case RBC:
      c = new RedBloodCell(creationTime);
      break;
    case WBC:
      c = new WhiteBloodCell(creationTime);
      break;
    case BAC:
      c = new Bacterium(creationTime);
      break;
    case PEN:
      c = new Penicillin(creationTime);
      break;
  }
...
void Inspector::divideBacterium(const int k, const int j)
{
  int kk, jj;
  randomUnoccupiedAdjacent(k, j, kk, jj);
  if((kk!=-1) && (jj!=-1))
  {
    if(blood(k, j)==0) cout << "DERP" << endl;
    Bacterium *toCopy = static_cast<Bacterium*>(blood(k, j)->getValue());
    Bacterium *copy = new Bacterium(*toCopy); //->*
...
Node<BloodEntity> *Inspector::action(const int k, const int j)
{
  ...
  IncAgeResult ageResult = thisEntity->incAge();
  ...
  if((jj>=0) && (jj<BLOOD_SIZE) && (kk>=0) && (kk<BLOOD_SIZE))
  {
    ...
    else //otherwise, interact with whatever's there
    {
      ...
      destroyed = thisEntity->interact(otherEntity);
      switch(destroyed)
      {
        case THIS:
          removeBloodEntity(k, j);
